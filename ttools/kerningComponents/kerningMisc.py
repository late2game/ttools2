...
def checkPairFormat(value):
    assert isinstance(value, tuple), 'wrong pair format'
    assert len(value) == 2, 'wrong pair format'
    assert isinstance(value[0], str), 'wrong pair format'
    assert isinstance(value[1], str), 'wrong pair format'
...
def buildPairsFromString(uniString, aFont):
    ...
    for eachI in range(1, len(splittedText)):
        myPair = (u'{}'.format(splittedText[eachI-1]), '{}'.format(splittedText[eachI]))
        pairs.append(myPair)
    return pairs
...
def loadKerningTexts(kerningTextFolder):
    ...
    for eachKerningTextBaseName in kerningTextBaseNames:
        ...
        for rawWord in kerningWordsDoc:
            if '#' in rawWord:
                word = u'{}'.format(rawWord[:rawWord.index('#')].strip())
            else:
