...
def loadSmartSets():
    ...
    return sorted(list(smartSets.items()), key=lambda x: x[0])
...
class VisualReporter(BaseWindowController):
    ...
    def _drawHeader(self, quota, fontStyles):
        ...
        for eachTitle, eachX in list(headers.items()):
            db.text(eachTitle, (PDF_MARGIN+eachX, quota))

    def _drawReport(self, referenceFont, someFonts, glyphNames, reportPath, caption):
        assert isinstance(reportPath, str) or isinstance(reportPath, str), 'this should be a string or unicode'
