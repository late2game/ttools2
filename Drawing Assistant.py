...
from vanilla import TextBox, EditText, ColorWell
...
COLOR_ONE_INIT = (0.88, 1, 0.4, 1)
COLOR_TWO_INIT = (0.4, 1, 0.64, 1)
COLOR_THREE_INIT = (0.4, 0.64, 1, 1)
COLOR_FOUR_INIT = (0.88, 0.4, 1, 1)
COLOR_FIVE_INIT = (1, 0.4, 0.4, 1)
INIT_COLORS = [COLOR_ONE_INIT, COLOR_TWO_INIT, COLOR_THREE_INIT, COLOR_FOUR_INIT, COLOR_FIVE_INIT]
...
class MultipleGridController(Group):
    ...
    def __init__(self, posSize, gridActive, ctrlsAmount, activeCtrls, callback):
        ...
        self.gridsDB = [{'horizontal': False, 'vertical': False, 'step': None, 'color': color} for color in INIT_COLORS]

        jumpingY = 4
        self.gridActiveCheck = CheckBox((0, jumpingY, NET_WIDTH*.6, vanillaControlsSize['CheckBoxRegularHeight']),
                                        "Show grids",
        ...
        for eachI in range(1, ctrlsAmount+1):
            ...
            gridCtrl = SingleGridController((0, jumpingY, NET_WIDTH, vanillaControlsSize['EditTextRegularHeight']),
                                            index=eachI,
                                            isVertical=False,
                                            isHorizontal=False,
                                            step=None,
                                            gridColor=INIT_COLORS[eachI-1],
...
class SingleGridController(Group):
    ...
    def __init__(self, posSize, index, isVertical, isHorizontal, step, gridColor, callback):
        ...
        # ctrls
        jumpinX = 12
        self.indexText = TextBox((jumpinX, 0, 16, vanillaControlsSize['TextBoxRegularHeight']), '%d)' % index)
        jumpinX += self.indexText.getPosSize()[2]

        self.stepCtrl = EditText((jumpinX, 0, 24, vanillaControlsSize['EditTextRegularHeight']),
                                 callback=self.stepCtrlCallback)
        jumpinX += self.stepCtrl.getPosSize()[2] + 16

        self.isHorizontalCheck = CheckBox((jumpinX, 0, 32, vanillaControlsSize['CheckBoxRegularHeight']),
                                          "H",
                                          value=self.isHorizontal,
                                          callback=self.isHorizontalCheckCallback)
        jumpinX += self.isHorizontalCheck.getPosSize()[2]+2

        self.isVerticalCheck = CheckBox((jumpinX, 0, 32, vanillaControlsSize['CheckBoxRegularHeight']),
                                        "V",
                                        value=self.isVertical,
                                        callback=self.isVerticalCheckCallback)
        jumpinX += self.isVerticalCheck.getPosSize()[2]+10

        self.whichColorWell = ColorWell((jumpinX, 0, 46, self.ctrlHeight),
...
class DrawingAssistant(BaseWindowController):
    ...
    def __init__(self):
        ...
        # init views
        self.bcpController = BcpController((MARGIN_HOR, 0, NET_WIDTH, vanillaControlsSize['CheckBoxRegularHeight']*2),
        ...
        # collect views
        accordionItems = [
                {'label': 'bcp ctrl', 'view': self.bcpController, 'size': vanillaControlsSize['CheckBoxRegularHeight']*2.5, 'collapsed': False, 'canResize': False},
                {'label': 'grid ctrl', 'view': self.gridController, 'size': 190, 'collapsed': False, 'canResize': False},
            ]
...
class BcpController(Group):
    ...
    def __init__(self, posSize, sqrActive, bcpLengthActive, callback):
        ...
        jumpingY = 2
        self.sqrCheck = CheckBox((0, jumpingY, NET_WIDTH, vanillaControlsSize['CheckBoxRegularHeight']),
                                 "Show squarings",
                                 value=self.sqrActive,
                                 callback=self.sqrCheckCallback)

        jumpingY += vanillaControlsSize['CheckBoxRegularHeight']
        self.bcpLengthCheck = CheckBox((0, jumpingY, NET_WIDTH, vanillaControlsSize['CheckBoxRegularHeight']),
                                       "Show bcp length",
